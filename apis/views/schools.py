...
from models.school_structure import SchoolLevel, Grade, Room
from models.student import Student
from models.student_score import Score
from models.subject import Subject
from models.teacher import Teacher
from logic.calculate_grade import CalculateGrade
...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not isinstance(first_name, str) or not isinstance(last_name, str) or not isinstance(subject_title, str):
            return Response({"detail": "First name, last name, and subject title must be strings."}, status=status.HTTP_400_BAD_REQUEST)

        if not isinstance(score, (int, float)) or not (0 <= score <= 100):
            return Response({"detail": "Score must be a number between 0 and 100."}, status=status.HTTP_400_BAD_REQUEST)
        
        if not isinstance(credit, int) or not (0 < credit <= 3):
            return Response({"detail": "Credit must be an integer between 1 and 3."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if not subject:
            return Response({"detail": "Subject not found."}, status=status.HTTP_400_BAD_REQUEST)
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        student_score = Score.objects.filter(student_id=kwargs.get("student_id", None)).values("subject__title", "subject__credit", "score", "student__first_name", "student__last_name", "student__school__name")
        student_score = list(student_score)


        subject_details = []
        for score in student_score:
            subject_details.append({
                "subject": score["subject__title"],
                "credit": score["subject__credit"],
                "score": score["score"],
                "grade": CalculateGrade.calculate_grade(score["score"])
            })

        data_pattern = {
            "student": {
                "id": kwargs.get("student_id", None),
                "full_name": student_score[0]["student__first_name"] + " " + student_score[0]["student__last_name"],
                "school": student_score[0]["student__school__name"]
            },
            "subject_detail": subject_details,
            "grade_point_average": CalculateGrade.calculate_gpa(subject_details)
        }
...
class PersonnelDetailsAPIView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        if not school_title:
            return Response({"detail": "School title is required."}, status=status.HTTP_400_BAD_REQUEST)

        school = School.objects.filter(name=school_title).first()
        if not school:
            return Response({"detail": "School not found."}, status=status.HTTP_404_NOT_FOUND)
        
        classes = Classes.objects.filter(school=school)
        teachers = Teacher.objects.filter(class_assigned__in=classes)
        students = Student.objects.filter(class_assigned__in=classes)
        personnel_details = []
        for i, teacher in enumerate(teachers, start=1):
            personnel_details.append(f"{i}. school: {school.name}, role: Teacher, class: {teacher.class_assigned.id}, name: {teacher.first_name} {teacher.last_name}")

        for i, student in enumerate(students, start=len(teachers)+1):
            personnel_details.append(f"{i}. school: {school.name}, role: Student, class: {student.class_assigned.id}, name: {student.first_name} {student.last_name}")
        ...
        return Response(personnel_details, status=status.HTTP_400_BAD_REQUEST)
...
class SchoolHierarchyAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        for school in schools:
                ...
                for class_ in classes.filter(school=school):
                    class_data = {
                        "class": class_.name,
                        "teachers": []
                    }
                    for teacher in teachers.filter(class_assigned=class_):
                        class_data["teachers"].append({
                            "teacher": f"{teacher.first_name} {teacher.last_name}",
                            "students": []
                        })
                        for student in students.filter(class_assigned=class_):
                            class_data["teachers"][-1]["students"].append(f"{student.first_name} {student.last_name}")
